...
class AzureHandler:
    ...
    def get_test_data(self, project_id, plan_id, suites_id:list = []):
        ...
        for suite_id in suites_id:
            ...
            test_data.extend(self.get_test_report_details(suite))

        return test_data

    def get_test_report_details(self, test_suite:TestSuite) -> Generator:

        project: TeamProjectReference = test_suite.project
        plan: TestPlanReference = test_suite.plan
        test_points = self.get_test_cases(project.id, plan.id, test_suite.id)
        self.logger.info(f"Test Report Details for Suite: {test_suite.name} (ID: {test_suite.id}) found {len(test_points)} test points.")

        for test_point in test_points:
            test_suite_id = test_point.test_suite.id
            test_suite_name = test_point.test_suite.name
            test_case_id = test_point.test_case_reference.id
            test_case_name = test_point.test_case_reference.name
            tp_result = test_point.results
            outcome = tp_result.outcome

            config_name = test_point.configuration.name
            config_value = test_config = self.test_plan_client.get_test_configuration_by_id(project.id, test_point.configuration.id)
            config_value = test_config.name if test_config else "Nessuna"

            test_run = self.test_client.get_test_results(project=project.id, run_id= tp_result.last_test_run_id) if tp_result.last_test_run_id else None
            notes = test_run[0].comment if test_run and len(test_run) > 0 else ""
            self.logger.debug(f"Test Case ID: {test_case_id}, Name: {test_case_name}, Outcome: {outcome}, Config: {config_name} ({config_value}), Notes: {notes}")

            yield (test_suite_id, test_suite_name, test_case_id, test_case_name, outcome, config_name, config_value, notes)
